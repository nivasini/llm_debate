#!/usr/bin/env python3
"""four_settings_cot_4omini.py

Four-settings consultant-motivation comparison (1=Aligned, 2=Correctness,
3=Both-goals, 4=Strategic) on gpt-4o-mini with explicit chain-of-thought
guidance.

Differences from scripts/four_settings_4omini.py:
  * Adds a 200-word limit on <argument> in all settings.
  * <thinking> instructions are explicit, non-sequential:
      (a) reasoning about what answer to provide and why
      (b) how to structure the explanation respecting the word limit
    Settings 2/3/4 receive both (a) and (b) in a single <thinking> block.
    Setting 1 splits them: Call A (choice) gets (a), Call B (explanation)
    gets (b) (Call B previously had no <thinking> block at all).

Output: exp/four_settings_cot/  (separate from exp/four_settings/ so the
two prompt variants can be compared.)
"""
...
OUT_DIR = Path("exp/four_settings_cot")
...
# ─── API helpers / generation / judging — same as four_settings_4omini ──────
...
async def main():
    ...
    parser.add_argument(
        ...
        help=f"Comma-separated settings to run. Default: all of {SETTINGS}.",
    )
    parser.add_argument(
